...
export const getOne = async (req, res) => {
  ...
  try {
    ...
    if (resultado.length > 0) {
      ...
      await postLog(`Consulta a ${tabla}`, `Consulta SELECT ONE a la ${tabla} = ${identificador}`);
    } else {
...
export const edit = async (req, res) => {
  ...
  if (!n_tarjeta || !fecha_caducidad || !ccv || !nombre_tarjeta || !rut_usuario) {
    return res.status(400).json({ msg: 'Todos los campos son obligatorios para la actualización' });
  }

  const updateQuery = `UPDATE ${tabla} SET n_tarjeta = ?, fecha_caducidad = ?, ccv = ?, nombre_tarjeta = ? WHERE ${identificador} = ?`;

  try {
    const [resultado] = await pool.query(updateQuery, [n_tarjeta, fecha_caducidad, ccv, nombre_tarjeta, rut_usuario]);

    if (resultado.affectedRows > 0) {
      res.json({ msg: 'Tarjeta actualizada correctamente' });
      await postLog(`Consulta a ${tabla}`, `Consulta UPDATE a la ${tabla} = ${rut_usuario}`);
    } else {
...
export const addCash = async (req, res) => {
  ...
  const { rut_usuario, n_ticket, monto_actual } = req.body;

  if (!rut_usuario || !n_ticket || !monto_actual) {
    return res.status(400).json({ msg: 'Todos los campos son obligatorios para realizar el pago' });
  }

  const updateQuery = `UPDATE ${tabla} SET monto = ? WHERE rut_usuario = ?`;

  try {
    const [resultado] = await pool.query(updateQuery, [monto_actual, rut_usuario]);

    if (resultado.affectedRows > 0) {
      res.status(200).json({ msg: 'Pago realizado correctamente' });
      await statusTicket(n_ticket);
      await postLog(`Consulta a ${tabla}`, `Consulta UPDATE a la ${tabla} = ${rut_usuario}`);
    } else {
      res.status(404).json({ msg: 'Tarjeta no encontrada' });
    }
