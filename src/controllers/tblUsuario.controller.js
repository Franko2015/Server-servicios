...
export const edit = async (req, res) => {
  ...
  try {
    const [resultado] = await pool.query(
      `UPDATE ${tabla} SET nombre = ?, apellido_paterno = ?, apellido_materno = ?, correo = ?, usuario = ?, contrasena = ?, nacionalidad = ?, estado_cuenta = ?, tipo_cuenta = ? WHERE ${identificador} = ?`,
      [
        nombre,
        apellido_paterno,
        apellido_materno,
    ...
    if (resultado.affectedRows > 0) {
      ...
      await postLog(
        `Consulta a ${tabla}`,
        `Consulta UPDATE a la ${identificador} = ${rut}`
      );
    } else {
      res.status(404).json({ msg: "No encontrado" });
    }
...
export const state = async (req, res) => {
  ...
  try {
    ...
    if (resultado.affectedRows > 0) {
      ...
      await postLog(`Consulta a ${tabla}`, `Suspensión de cuenta a ${rut}`);
    } else {
...
export const create = async (req, res) => {
  ...
  try {
    const [resultado] = await pool.query(
      `SELECT * FROM ${tabla} WHERE ${identificador} = ? OR usuario = ? OR correo = ?`,
      [rut_usuario, usuario, correo]
    );
    ...
    if (resultado && resultado.length > 0) {
      ...
      await postLog(
        `Create de usuario ${nombre}`,
        `Usuario ${rut_usuario} creado correctamente`
      );
